...
async def set_timer(meeting, alarm):
    ...
    await announce(f"Meeting '{meeting.description}' for {meeting.user_list} starts now")
...
async def notify_meeting(meeting, notification, minutes_remaining):
    ...
    announcement = announce(f"Meeting '{meeting.description}' for {meeting.user_list} starts in {minutes_remaining} minutes")
...
async def check_upcoming_meeting(meeting):
    ...
    # give the MINUTE notification and prepare the meeting channel
        ...
        notification = notify_meeting(meeting, database.Notification.MINUTE, minutes_remaining)
    ...
    if len(meetings) == 0:
        ...
        for meeting in meetings:
            meeting_dt = parse(str(meeting.date_time),
                               settings={'TO_TIMEZONE': time_zone, 'RETURN_AS_TIMEZONE_AWARE': False})
...
async def meeting_create_cmd(ctx, description, participants, time):
    ...
    db_call = database.add_meeting(description, iso_time, participants)
